...
def sell_ema_breakout(data, ema):
    if data['close'].iloc[-2] < data[ema].iloc[-2]:
...
def buy_ema_violation(data, ema):
    if data['high'].iloc[-2] > data[ema].iloc[-2]:
        return True
    return True

def sell_ema_violation(data, ema):
    if data['low'].iloc[-2] < data[ema].iloc[-2]:
        return True
    return False

def buy_double_ema_breakout(data, ema1, ema2):
    if data['close'].iloc[-2] > data[ema2].iloc[-2]:
        if data['close'].iloc[-2] > data[ema1].iloc[-2]:
...
def buy_double_ema_violation(data, ema1, ema2):
    if data['high'].iloc[-2] > data[ema2].iloc[-2]:
        if data['high'].iloc[-2] > data[ema1].iloc[-2]:
            return True
    return False

def sell_double_ema_violation(data, ema1, ema2):
    if data['low'].iloc[-2] < data[ema2].iloc[-2]:
        if data['low'].iloc[-2] < data[ema1].iloc[-2]:
            return True
    return False

def buy_triple_ema_breakout(data, ema1, ema2, ema3):
    if data['close'].iloc[-2] > data[ema3].iloc[-2]:
        if data['close'].iloc[-2] > data[ema2].iloc[-2]:
            if data['close'].iloc[-2] > data[ema1].iloc[-2]:
                if data['high'].iloc[-1] > data['high'].iloc[-2]:
                    return True
...
def buy_triple_ema_violation(data, ema1, ema2, ema3):
    if data['high'].iloc[-2] > data[ema3].iloc[-2]:
        if data['high'].iloc[-2] > data[ema2].iloc[-2]:
            if data['high'].iloc[-2] > data[ema1].iloc[-2]:
                return True
    return False
...
def sell_triple_ema_violation(data, ema1, ema2, ema3):
    if data['low'].iloc[-2] < data[ema3].iloc[-2]:
        if data['low'].iloc[-2] < data[ema2].iloc[-2]:
            if data['low'].iloc[-2] < data[ema1].iloc[-2]:
                return True
    return False